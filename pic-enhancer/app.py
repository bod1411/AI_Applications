...
def enhance_image_replicate(image_file, scale_factor=4):
    ...
    try:
        ...
        # Higher scale factors need smaller input images to avoid GPU OOM
        if scale_factor == 4:
        ...
        if current_pixels > MAX_PIXELS:
            ...
            st.info(f"💡 Tip: For large images, try 2x scale first for better success rate!")
...
def main():
    ...
    with st.sidebar:
        ...
        **💡 Tips for Success:**
        - **Large images (>5MP):** Use 2x scale
        - **Medium images (2-5MP):** Try 2x or 3x
        - **Small images (<2MP):** All scales work
        - **If GPU error:** Switch to Local Enhancement
        """)
    ...
    with col1:
        ...
        if uploaded_file is not None:
            ...
            if total_pixels > 5_000_000:  # > 5MP
                ...
                st.markdown("- Start with **2x scale**")
                ...
                with st.spinner("Enhancing your image... This may take a minute."):
                    ...
                    if error:
                        ...
                        if "CUDA out of memory" in str(error) or "memory" in str(error).lower():
                            ...
                            st.markdown("""
                            1. **Use 2x scale instead of 4x** (requires less GPU memory)
                            2. **Try Local Enhancement method** (no GPU needed)
                            3. **Resize your image smaller** before uploading
                            4. **Wait a minute and try again** (GPU might be busy with other users)
                            """)
                        else:
                            st.info("💡 Try using the Local Enhancement method instead.")
